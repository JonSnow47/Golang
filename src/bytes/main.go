/*
 *  Revision History:
 *      Initial: 2018/07/23    Chen Yan Chen
 */
...
// bytes.Contains 检测子片 subslice 是否在 b 内
func Contains() {
	b := []byte("google")
	subslice := []byte("oo")
	fmt.Println(bytes.Contains(b, subslice))
}

// bytes.LastIndex 返回匹配字符前有几个字符未被匹配，若字符不被匹配则返回-1
func LastIndex() {
	s := []byte("google")
	sep := []byte("aoog")
